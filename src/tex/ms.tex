...
\begin{equation}
    \hat{A}_m = \frac{\Re \left\langle \tilde{h}_m(\theta) \mid r(\theta) \right\rangle}{\left\langle \tilde{h}_m(\theta) \mid \tilde{h}_m(\theta) \right\rangle},
\end{equation}
...
\begin{multline}
    \log \bar{\mathcal{L}} = \log \int \dif A_m \exp\left( \log \mathcal{L} \right) = \\ -\frac{1}{2} \left( \left\langle r(\theta) \mid r(\theta) \right\rangle - \hat{A}_m \Re \left\langle \tilde{h}_m(\theta) \mid r(\theta) \right\rangle \right) \\ - \frac{1}{2}\log\left( 2 \pi \left\langle \tilde{h}_m \mid \tilde{h}_m \right\rangle \right) + C.
\end{multline}
...
\begin{multline}
    \log w_i = \log \bar{\mathcal{L}}(\theta_i) - \log \mathcal{L}(\theta_i) = \\ \frac{1}{2} \hat{A}_m \Re \left\langle \tilde{h}_m(\theta_i) \mid r(\theta_i) \right\rangle \\ - \frac{1}{2}\log\left( 2 \pi \left\langle \tilde{h}_m \mid \tilde{h}_m \right\rangle \right).
\end{multline}
...
Keefe can supply the first component.  Note that the above computation
\emph{does not require the log-likelihood from the original posterior
estimation}, but only the samples drawn from the original posterior, and the
ability to compute the detector response and the inner products.  This means
that we can apply this method to any existing posterior samples, regardless of
how they were generated, as long as we can compute the detector response and the
inner products for each sample.  It may be easiest to simply compute the above
using codes of our own (effectively ignoring calibration parameters, etc) rather
than trying to duplicate the Bilby setup?
...
We simultaneously model the memory and all the usual astrophysical parameters following the hierarchical framework of \citep{Payne2023}.
We assume we start from regular PE samples $\theta_i$ that have been enhanced with $\hat{A}_m$ and $\sigma_{A_m}$, henceforth $\mu_i$ and $\sigma_i$ for each sample, as described above.
This defines the conditional distribution on $A$ given $\theta_i$, that is
\begin{equation}
    p(A \mid \theta_i) = \mathcal{N}(A \mid \mu_i, \sigma_i).
\end{equation}
We can leverage this within our hierarchical integral to marginalize over $A$ analytically without the usual Monte Carlo sum.

The hierarchical likelihood for population parameters $\Lambda$ requires computing an integral of the form
\begin{equation}
    I = \int \frac{p(A, \theta)}{W(\theta)}\, p(A, \theta \mid \Lambda)\, \dif A \dif \theta \, .
\end{equation}
Where $p(A, \theta)$ is the posterior and $W(\theta)$ is an importance weight that encodes the sampling prior and any necessary Jacobians;
we will assume that the prior on $A$ is flat, so that $W$ is independent of $A$.
Since $p(A, \theta) = p(A\mid\theta)\,p(\theta)$, we can rewrite the above as
\begin{equation}
    I = \int p(A\mid\theta)\,p(\theta)\,p(A, \theta \mid \Lambda)\,{\rm d}\theta\,{\rm d}A \, ,
\end{equation}
where we have further assumed that the population model factorizes as $p(A, \theta) = p(A\mid\theta)\,p(\theta)$.

We can now use the fact that we have $N$ samples from the posterior $\theta_i$ to approximate the integral as
\begin{align}
    I &\approx \int \frac{1}{N} \sum_i p(A\mid\theta_i)\,\delta(\theta-\theta_i)\,\frac{p(A, \theta \mid \Lambda)}{W(\theta)}\,{\rm d}\theta\,{\rm d}A \, , \nonumber\\
    &= \frac{1}{N} \sum_i \int p(A\mid\theta_i)\,\frac{p(A, \theta_i \mid \Lambda)}{W(\theta_i)}\, {\rm d}A \, .
\end{align}
If we further assume that the population model factorizes as $p(A, \theta \mid \Lambda) = p(A\mid \Lambda_A)\,p(\theta\mid\Lambda_\theta)$, then
\begin{equation}
    I \approx \frac{1}{N} \sum_i \frac{p(\theta_i\mid\Lambda_\theta)}{W(\theta_i)} \int p(A\mid\theta_i)\,p(A\mid \Lambda_A)\,\,{\rm d}A \, .
\end{equation}
Now further assume a Gaussian population model on $A$ with mean $\mu_\Lambda$ and standard deviation $\sigma_\Lambda$, so that $p(A\mid \Lambda_A) = \mathcal{N}(A \mid \mu_\Lambda, \sigma_\Lambda)$; then the integral over $A$ can be performed analytically, yielding
\begin{align}
    I &\approx \frac{1}{N} \sum_i \frac{p(\theta_i\mid\Lambda_\theta)}{W(\theta_i)} \int \mathcal{N}(A \mid \mu_i, \sigma_i)\,\mathcal{N}(A \mid \mu_\Lambda, \sigma_\Lambda)\,{\rm d}A \, , \nonumber\\
    &= \frac{1}{N} \sum_i \frac{p(\theta_i\mid\Lambda_\theta)}{W(\theta_i)}\,\mathcal{N}(\mu_i \mid \mu_\Lambda, \sqrt{\sigma_i^2 + \sigma_\Lambda^2})\, .
\end{align}
This is the final expression we need to compute the hierarchical likelihood.
Everything else is as in a usual populations analysis.

We will assume that there are no selection effects on $A$, which is a reasonable assumption \citep{Magee2024}.
